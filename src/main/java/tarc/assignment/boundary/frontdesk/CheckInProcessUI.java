...
public class CheckInProcessUI implements UI {
    ...
    public void run(){
        try {
            ...
            boolean meal=app.input().readYesNo("Add breakfast meal set [RM 10.00/day] ? ");
            availableRoomUI.run();
            String roomNum=app.input().readString("Your room number selection :");
            boolean result=app.checkInController().checkIn(confirmationNum,roomNum,customerId,meal,day);
            if (result){
